...
This method is one of the most preferred methods for online upgrades because it
does not require any additional nodes to be added to the cluster for the upgrade.
The method is fast and less resource intensive because the node is recovered by
using only delta changes instead of a full rebalance. Also, the global secondary
indexes are preserved in this process, which does not require a rebuild. The
primary disadvantage of this approach is decreased high availability for the
duration of upgrade.
...
Use the Console to log in to cluster `node-1` by using the `Adminstrator`
...
cluster. The following images shows we have two server nodes that are part of
this cluster:
...
first so that the console session remains intact even after the Couchbase
...
The following image shows the console after the upgrade completes:
...
After the shutdown, the status of the node on the console changes to **Node
unresponsive** as shown in the following image:
...
The console output now shows the upgraded version as well as the option to add
back using either **Full Recovery** or **Delta Recovery**, as shown in the
following image:
...
After Rebalance completes, the node turns green in the console and is back in
sync on the cluster. You can also see the version is upgraded to 5.5 as shown
in the following image:
...
log in to the `node-2` Console as `Administrator` and repeat Steps 5 to 9 for
Server `node-1`.

The following image shows the Console after the upgrade and recovery of `node-1`
is complete:

![]({% asset_path 2018-12-05-rolling-upgrade-of-couchbase-server-using-graceful-failover-option/Picture10-1.png %})

Note the two new options on the left side menu, which are new in Version 5.5.0.

- Analytics: Enables execution of familiar SQL queries by using N1QL for analytics.

- Eventing: Enables us creation of server-side functions, which are triggered
...
syncing only the changes which took place during the upgrade. This is
significantly less time consumming.
...
To take maximum advantage of this approach, plan the upgrade should in off-peak
hours. This addresses the only shortcoming of this approach (decreased high
availability) and speeds up the upgrade because the delta changes will be much
fewer during off-peak hours.
