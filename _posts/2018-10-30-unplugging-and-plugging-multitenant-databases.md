...
The post discusses detaching one pluggable database (PDB) from the source
container database (CDB) and attaching it to a target CDB. For the purposes of
this post, the PDB is the database in which we save all our application-related
databases.
...
sales department data in a sales application and HR department data in an HR
application.

To unplug a PDB means to detach a PDB database from the source CDB and attach
it to a target CDB.

In this post, the source CDB, `TEST12`, has two PDBs, `PDB1` and `PDB2`. The
target CDB, `DEV12`, receives the unplugged PDBs.

### Unplug and plug PDB sample

In this section, you learn how to unplug `PDB1` and `PDB2` from source CDB
`TEST12` and plug them into target CDB `DEV12`.
...
Before starting the plugging operation, ensure that the to-be-plugged-in PDB is
...
data files of the unplugged PDB.
...
You can plug the PDB into the target CDB using either the copy method, the
non-copy, or the clone move method.

##### Plug with the copy method

Tho use the copy method to plug the PDB, create and define a destination for
the new data files, plug the unplugged PDB into the target CDB, and then copy
the data files of the unplugged PDB.

Create destination directory by running the following code:

    mkdir /home/oracle/app/oradata/DEV12/pdb1
    sqlplus / as sysdba

Use the data files of the unplugged PDB to plug the PDB into the CDB and copy
the data files to a new location by running the following code:
...
name as the temp file specified in the XML file present in the target location.
...
An error message pops up because the GUID is not unique. `PDB1` was already
plugged using the copy method method, which caused this conflict with the GUID
...
To open the PDB and finalize the plugging operation, run the following commands:
...
servers and reduce the chances of human error.For example, you can apply patch
in one shot for all PDB databases, whereas in a non-container environment you
need to apply patch to all databases separately. Backup and other maintenance
