...
3. Link the new function to the VPD policy, as shown in the following image:
    ...
    You may use the following VPD options:

    - **sec_relevant_cols**: This option enables column-level VPD, which
    ...
    - **sec_relevant_cols_opt**: Use with parameter with the
      `sec_relevant_cols` parameter to display all rows for column-level VPD,
      filtered queries (`SELECT` only), where sensitive columns appear as
