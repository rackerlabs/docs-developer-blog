...
and policies. The benefits of tape virtualization include storage
consolidation and faster data restore processes. For most mainframe data
centers, the storage capacity varies. However, protecting their business and
mission-critical data is always vital.

In addition, disk technology doesn't rely on streaming and can write
effectively regardless of data transfer speeds. This means that switching to
VTLs also eliminates streaming problems that often impair efficiency in tape
drives.
...
found to be faster than backup, regardless of implementation.
...
Instead of reading from and writing to tape directly, VTLs maintain a large disk-based buffer of virtual tapes. This buffer expedites the read and write
processes. This buffer often represents as many as 256 or 512 tapes.

The next sections describe the benefits of this approach.
...
With simple, automated tape libraries, storage devices were often under-used.
...
yet, you can devise a way to optimize file storage across many virtual
devices. For example, you can save space by copying several smaller virtual
tapes to one physical tape.

### mhVTL

mhVTL is a free virtual tape library. Because this library isn't
production-ready, you can use it to explore VTL and for non-production
...
Let's walk through an example that might help you understand mhVTL a bit
better.

Imagine you're using two virtual box Centos machines. The first represents the
VTL and the second represents the backup machine. One Centos machine is
running VTL.
...
13. To test whether the web-based user interface (UI) is accessible from your
    browser, paste the following URL into your browser:

          http://192.168.171.142/mhvtl/

    You should see the screen that's shown in the following image:

    ![Screenshot of UI login page]({% asset_path 2018-07-27-vtls/picture1.png %})

13. Log into the UI. The default password is **MHVTL**.
...
3. In the **Select Library to remove** drop-down box, select a single library
   at a time (if there are multiple libraries), select **Yes** on **Remove all
   tape media also**, and click **Continue**.

4. The last robot can't be removed, but once you've created your own robot you
   can go ahead and remove the default one.

   ![Screenshot of UI: Removing a library]({% asset_path 2018-07-27-vtls/picture2.png %})

5. After you've removed the robot, click **Start MHVTL**.

6. Next, click **Setup** in the left navigation pane, then click **Add >
   Standard**. These steps will initiate the setup wizard.

7. Select **Next** to begin working with the wizard.

#### Create a tape library

In this example, we'll create an Oracle&reg; StorageTek L700 tape library.

1. Select **STK**, then fill out all of the required settings.

   ![Screenshot of UI: Creating a StorageTek tape library on the MVHTL Settings page]({% asset_path 2018-07-27-vtls/picture3.png %})

2. After you've entered all of the details, click the **Submit** button, then
   click **Finish** twice. Finally, click **Return**.

3. Select **Console** in the left navigation pane and click the **Start**
...
Now let's configure the robot STK L700 as an iSCSI target to prepare it to be
...
#### Use backup software with your virtual tape library

Our basic setup is complete, and we're ready to use any backup software with
our tape library.

In this example, we work with the very basic Linux utilities ``mt`` and
``mtx``.
...
- ``/dev/rmt/0`` or ``/dev/rmt/1`` or ``/dev/rmt/[0-127]``: This is the
  standard tape device name on Unix. The tape is rewound.

- ``/dev/rmt/0n``: Tapes with this name are referred to as no-rewind. After
  the tape is used, the tape remains in the current status for next command.

- ``/dev/rmt/0b``: Use magtape interface i.e. BSD behavior. More readable by a
  variety of operating systems, including AIX, Windows, Linux, and FreeBSD.
...
- The name ``/dev/rmt/1cn`` indicates that I’m using unity 1, compressed
  density and no rewind.

- The name ``/dev/rmt/0hb`` indicates that I’m using unity 0, high-density and
  BSD behavior.
...
To accomplish this task, we have go back to the MHVTL portal and load the tape
on slot 0.

1. Select **Operator > Mount**.

   ![Screenshot of UI showing the locations of the Operator and Mount options]({% asset_path 2018-07-27-vtls/picture4.png %})

2. Select **Element Slot** and **Drive**.

   ![Screenshot of UI showing the locations of the Slot and Drive options]({% asset_path 2018-07-27-vtls/picture5.png %})

3. Next, go back to ``server3`` (the backup server) and check its status again.
...
**Back up the directory (tar format)**

To back up the directory in TAR format, enter the following command:
...
To pull up the tape contents in tape archive (TAR) format, enter the following
command:
...
To pull up the tape contents in TAR format, enter the following commands:
...
There are many other options for restoring a directory, such as a partition
backup, or using free backup software such as Bacula&reg;.

### Conclusion

Managing physical tape backup requires both physical effort, time and involves
costs in terms of setting up and sending tapes offsite. This option is not
feasible for small and medium size business for taking data backup.

Likewise, retrieving data from tape backup is inconvenient and searching for
correct tape is also a tedious process.
...
performance that enables you to integrate newer data encryption techniques
with disk. A virtual tape library emulates the tape drive and gives a feel of
a physical tape. VTL combines the advantages of both real physical tapes and
disks and delivers high speed backup with restore management in case of any
loss.
...
* A non-disruptive, low-effort migration to new backup approach
...
* Low-cost data archiving for long-term retention, with predictable retrieval
  times

We hope you find this post useful. In case you have any questions on the topic
please feel free to reach out to me by clicking on the button below. You can
